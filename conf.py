...
if not odoo_sources_dirs:
    ...
    source_read_replace['ODOO_RELPATH'] = '/../' + str(odoo_sources_dirs[0])
    source_read_replace['ODOO_ABSPATH'] = str(odoo_dir)
...
def sourceReadReplace(app, docname, source):
    """Susbstitute parts of strings by computed value.
    ...
    This is using the config `source_read_replace`, mapping a name to its
    replacement.
    This will look for the name surounded by curly braces in the source.

    Meant to be connected to the `source-read` event.
    """
    result = source[0]
    for key in app.config.source_read_replace:
        result = result.replace(f"{{{key}}}", app.config.source_read_replace[key])
    source[0] = result
...
def setup(app):
    ...
    app.add_config_value('source_read_replace', {}, 'env')
    app.connect('source-read', sourceReadReplace)
